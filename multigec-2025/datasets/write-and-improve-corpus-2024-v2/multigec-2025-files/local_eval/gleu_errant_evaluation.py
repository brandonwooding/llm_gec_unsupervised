    ...
    with open(os.path.join(output_dir, 'scores.txt'), 'w') as output_file:
        ...
        for submission_file in submission_files:
            ...
            if submission_file_name.startswith(('cs-natform', 'CS-NATFORM')):
                ...
                ref_file = f'cs-natform-ref1-{data_split}.tmp'
                ref2_file = f'cs-natform-ref2-{data_split}.tmp'
                orig_file = f'cs-natform-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('cs-natwebinf', 'CS-NATWEBINF')):
                lang_code = "CS_NATWEBINF"
                ref_file = f'cs-natwebinf-ref1-{data_split}.tmp'
                ref2_file = f'cs-natwebinf-ref2-{data_split}.tmp'
                orig_file = f'cs-natwebinf-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('cs-romani', 'CS-ROMANI')):
                lang_code = "CS_ROMANI"
                ref_file = f'cs-romani-ref1-{data_split}.tmp'
                ref2_file = f'cs-romani-ref2-{data_split}.tmp'
                orig_file = f'cs-romani-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('cs-seclearn', 'CS-SECLEARN')):
                lang_code = "CS_SECLEARN"
                ref_file = f'cs-seclearn-ref1-{data_split}.tmp'
                ref2_file = f'cs-seclearn-ref2-{data_split}.tmp'
                orig_file = f'cs-seclearn-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('en', 'EN')):
                lang_code = "EN"
                ref_file = f'en-writeandimprove2024-ref1-{data_split}.tmp'
                orig_file = f'en-writeandimprove2024-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('et-eic', 'ET-EIC')):
                lang_code = "ET_EIC"
                ref_file = f'et-eic-ref1-{data_split}.tmp'
                ref2_file = f'et-eic-ref2-{data_split}.tmp'
                ref3_file = f'et-eic-ref3-{data_split}.tmp'
                orig_file = f'et-eic-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('et-ekil2', 'ET-EKIL2')):
                lang_code = "ET_EKIL2"
                ref_file = f'et-ekil2-ref1-{data_split}.tmp'
                ref2_file = f'et-ekil2-ref2-{data_split}.tmp'
                orig_file = f'et-ekil2-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('de', 'DE')):
                lang_code = "DE"
                ref_file = f'de-merlin-ref1-{data_split}.tmp'
                orig_file = f'de-merlin-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('el', 'EL')):
                lang_code = "EL"
                ref_file = f'el-glcii-ref1-{data_split}.tmp'
                orig_file = f'el-glcii-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('is-ec', 'IS-EC', 'is-IceEC', 'is-iceec')):
                lang_code = "IS_EC"
                ref_file = f'is-iceec-ref1-{data_split}.tmp'
                orig_file = f'is-iceec-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('is-l2ec', 'IS-L2EC', 'is-IceL2EC', 'is-icel2ec')):
                lang_code = "IS_L2EC"
                ref_file = f'is-icel2ec-ref1-{data_split}.tmp'
                orig_file = f'is-icel2ec-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('it', 'IT')):
                lang_code = "IT"
                ref_file = f'it-merlin-ref1-{data_split}.tmp'
                orig_file = f'it-merlin-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('lv', 'LV')):
                lang_code = "LV"
                ref_file = f'lv-lava-ref1-{data_split}.tmp'
                orig_file = f'lv-lava-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('ru', 'RU')):
                lang_code = "RU"
                ref_file = f'ru-rulec-ref1-{data_split}.tmp'
                orig_file = f'ru-rulec-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('sl', 'SL')):
                lang_code = "SL"
                ref_file = f'sl-solar_eval-ref1-{data_split}.tmp'
                orig_file = f'sl-solar_eval-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('sv', 'sw', 'SV')):
                lang_code = "SV"
                ref_file = f'sv-swell_gold-ref1-{data_split}.tmp'
                orig_file = f'sv-swell_gold-orig-{data_split}.tmp'
            elif submission_file_name.startswith(('uk', 'UK')):
                lang_code = "UK"
                ref_file = f'uk-ua_gec-ref1-{data_split}.tmp'
                ref2_file = f'uk-ua_gec-ref2-{data_split}.tmp'
                ref3_file = f'uk-ua_gec-ref3-{data_split}.tmp'
                ref4_file = f'uk-ua_gec-ref4-{data_split}.tmp'
                orig_file = f'uk-ua_gec-orig-{data_split}.tmp'
            else:
            ...
            if errant_scores != "":
                ...
                output_file.write(f"\n")
                newline = {'team_name': team_name, 'language': lang, 'corpus': corpus, 'split': data_split, 'track': minimal_fluency, 'language': lang, 'GLEU': gleu_score, 'precision': float(prec)*100, 'recall': float(rec)*100, 'F0.5': float(f05)*100}
                print(newline)
                print("=============================")
